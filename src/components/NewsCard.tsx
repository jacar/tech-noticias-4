...
import { Eye, Clock, ExternalLink, AlertCircle, CheckCircle, Share2, Bookmark } from "lucide-react";
...
export const NewsCard = ({
  ...
  // Use normalized URL from data
  const linkToOpen = news.url;

  // Uncomment below to debug links
  // useEffect(() => console.log(`[NewsCard] linkToOpen: ${linkToOpen}`), [linkToOpen]);
  ...
  return (
    <a
      ...
      className="group relative flex flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg dark:bg-slate-800 cursor-pointer"
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-4 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={e => { e.stopPropagation(); e.preventDefault(); handleSaveClick(e); }}
              className="flex items-center gap-1 text-pink-500 hover:text-pink-600 dark:text-pink-400 dark:hover:text-pink-300"
              aria-label={isSaved ? "Eliminar guardado" : "Guardar noticia"}
            >
              <Bookmark size={14} />
              <span>{isSaved ? "Guardado" : "Guardar"}</span>
            </button>
          </div>
          <div className={`flex items-center gap-1 ${news.isRssFeed ? 'text-green-600 dark:text-green-500' : urlStatus === 'invalid' || isCorrectDomain === false ? 'text-amber-600 dark:text-amber-500' : 'text-blue-600 dark:text-blue-400'} hover:underline`}>
      ...
      {/* Debug: show raw and computed links */}
      {showDebugInfo && (
        <div className="p-2 bg-yellow-200 text-xs text-black">
          <p>rawLink: {news.originalLink || news.url}</p>
          <p>linkToOpen: {linkToOpen}</p>
        </div>
      )}
    </a>
  );
